...
from load.get_or_create_domain import get_create_domain
from load.save_screenshot import save_screenshot
...
from extraction.take_screenshot import take_screenshot

@flow(name="main flow", retries=2, retry_delay_seconds=2)
async def main_flow(domain_id: UUID, domain: Domain):

    dns_records, http_md, screenshot_path = await asyncio.gather(fetch_dns_records(domain), fetch_http_header(domain), take_screenshot(domain, "domains_screenshots"))
    ...
    save_screenshot(screenshot_path,domain_id)
    


#the entrypoint to start the flow 
async def start_flow(domain: Domain):
    id = get_create_domain(domain)
    await main_flow(id, domain)
    

if __name__== "__main__":
    domain = Domain(name="google.com")
    asyncio.run(start_flow(domain))
